...
import {
  ...
  RegistryCompanyRegistry,
  RegistryProductDefinition
} from './types'

const REGISTRY_ROOT = join(process.cwd(), 'src', 'registry')
const LOCAL_OVERRIDES_ROOT = join(process.cwd(), 'local', 'overrides')

// Company and product paths (multi-org structure)
const COMPANY_ROOT = join(REGISTRY_ROOT, 'company')
const PRODUCT_ROOT = join(REGISTRY_ROOT, 'product')

// Default company ID (can be configured via environment)
...
const listFiles = (dirPath: string, extension: string): string[] =>
  listFilesWithExtensions(dirPath, [extension])

// Company file paths (multi-org: company/{companyId}/)
const COMPANY_METADATA_FILE = join(COMPANY_ROOT, DEFAULT_COMPANY_ID, 'metadata.json')
const COMPANY_REGISTRY_FILE = join(COMPANY_ROOT, DEFAULT_COMPANY_ID, 'registry.json')
...
const parseSkillDoc = (filePath: string): RegistrySkillDoc => {
  ...
  let id = relative(join(REGISTRY_ROOT, 'skills'), filePath)
    .replace(/\\/g, '/')
    .replace(/\.md$/, '')
  let title = id
  let tags: string[] = []
...
const ensureBusinessAlignedAgentTemplate = (
  ...
  const requiredRoleRequirement =
    'Enforce company global non-negotiables and human approval gates for critical actions'
  const productBoundaryRequirement =
    'Preserve strict separation between children education outputs and mature fiction outputs'
  ...
  const hasVisionAnchor =
    template.individual_vision.toLowerCase().includes('company vision') ||
    context.alignmentKeywords.some((keyword) =>
      template.individual_vision.toLowerCase().includes(keyword)
    )
  const hasCoreObjectiveAnchor =
    template.core_objective.toLowerCase().includes('company') ||
    context.alignmentKeywords.some((keyword) =>
      template.core_objective.toLowerCase().includes(keyword)
    )
...
const evaluateAgentAlignment = (
  ...
  const checkField = (
    ...
    const overlap = context.alignmentKeywords.filter((keyword) =>
  ...
  const roleAligned =
    roleReqText.includes('non-negotiable') &&
...
const normalizeAgentTemplate = (raw: unknown): RegistryAgentTemplate => {
  ...
  const workflowRefs = Array.isArray(source.workflows)
    ...
    : source.workflows && typeof source.workflows === 'object'
      ? Object.keys(source.workflows as Record<string, unknown>)
          .map((key) => key.trim())
          .filter((key) => key.length > 0)
      : []
  const dataRefs =
    toStringArray(source.data_requirements).length > 0
      ? toStringArray(source.data_requirements)
      : toStringArray(source.data)
  const derivedObjective =
    goalText.length > 0 ? goalText : 'Deliver role outcomes aligned with company mission.'
  const individualVision =
    typeof source.individual_vision === 'string' && source.individual_vision.trim().length > 0
      ? source.individual_vision.trim()
      : derivedObjective
  const roleNonNegotiables =
    toStringArray(source.role_non_negotiable_requirements).length > 0
...
const normalizeWorkflow = (
  ...
  const steps =
    toStringArray(source.steps).length > 0
  ...
  const dependenciesSource =
    source.dependencies && typeof source.dependencies === 'object'
      ? (source.dependencies as Record<string, unknown>)
      : {}

  const inferredWorkflowId = typeof source.id === 'string' ? source.id : fallbackId

  const inferredAgentId = typeof source.agent_id === 'string' ? source.agent_id : fallbackAgentId
    ...
    ? source.collaborators
        ...
        .map((entry) => {
          const collaborator: {
            ...
            responsibility:
              typeof entry.responsibility === 'string' ? entry.responsibility.trim() : ''
          }
        ...
        .filter(
          (entry) =>
            entry.agent_id.length > 0 && entry.role.length > 0 && entry.responsibility.length > 0
        )
    : []

  const handoffRulesSource =
    source.handoff_rules && typeof source.handoff_rules === 'object'
      ? (source.handoff_rules as Record<string, unknown>)
      : null

  const transferPoints =
    handoffRulesSource && Array.isArray(handoffRulesSource.transfer_points)
      ? handoffRulesSource.transfer_points
          ...
          .filter(
            (entry) =>
              entry.id.length > 0 &&
              entry.from_agent.length > 0 &&
              entry.to_agent.length > 0 &&
              entry.condition.length > 0 &&
              entry.context_packet_required_fields.length > 0
          )
      : []
  ...
  return {
    ...
    trigger:
      typeof source.trigger === 'string'
        ? source.trigger
        : typeof source.mode_trigger === 'string'
          ? source.mode_trigger
          : 'Manual invocation',
    workflow_mode:
      source.workflow_mode === 'global-collaborative' || source.workflow_mode === 'single-agent'
...
const buildAgentSetupFieldSchema = (
...
const buildKpiInputFieldSchema = (
...
const resolveByReference = <T>(
  ...
  for (const entry of entries) {
    const normalizedKeys = keys(entry).map(normalizeReference)
    if (
      normalizedKeys.some((key) => key.includes(normalizedValue) || normalizedValue.includes(key))
    ) {
      return entry
    }
  }
...
const parseProtocolYamlDoc = (filePath: string): RegistryProtocolDoc => {
  ...
  const rules =
    toStringArray(parsed.rules).length > 0
...
export const loadRegistrySnapshot = (version: number): RegistryLoadResult => {
  ...
  const moduleManifestSchema = readJson<Record<string, unknown>>(
    join(schemaRoot, 'module-manifest.schema.json')
  )
  const agentTemplateSchema = readJson<Record<string, unknown>>(
    join(schemaRoot, 'agent-template.schema.json')
  )
  const kpiMinimumSchema = readJson<Record<string, unknown>>(
    join(schemaRoot, 'kpi-minimum-requirements.schema.json')
  )
  const kpiLibrarySchema = readJson<Record<string, unknown>>(
    join(schemaRoot, 'kpi-definition.schema.json')
  )
  const dataInputLibrarySchema = readJson<Record<string, unknown>>(
    join(schemaRoot, 'data-input-definition.schema.json')
  )
  ...
  const companyCoreSchema = readJson<Record<string, unknown>>(
    join(schemaRoot, 'company-core.schema.json')
  )
  const productDetailsSchema = readJson<Record<string, unknown>>(
    join(schemaRoot, 'product-details.schema.json')
  )
  ...
  const company = existsSync(COMPANY_METADATA_FILE)
    ? readJson<RegistryCompanyCore>(COMPANY_METADATA_FILE)
    : defaultCompany
  ...
  const manifests: RegistryModuleManifest[] = manifestFiles.map((filePath) => {
    ...
    if (!valid) {
      ...
      validationErrors.push(
        `Invalid module manifest ${relative(REGISTRY_ROOT, filePath)}: ${details}`
      )
    }
    return parsed
  })
  ...
  const kpis: RegistryKpiDefinition[] = kpiWithSources.map(({ sourcePath, value }) => {
    ...
    if (!valid) {
      const details = (validateKpiLibraryEntry.errors ?? [])
        ...
        .join('; ')
      validationErrors.push(
        `Invalid KPI definition ${relative(process.cwd(), sourcePath)}: ${details}`
      )
    }
    return value
  })
  ...
  const dataInputs: RegistryDataInputDefinition[] = dataInputsWithSources.map(
    ({ sourcePath, value }) => {
      ...
      if (!valid) {
        const details = (validateDataInputLibraryEntry.errors ?? [])
          ...
          .join('; ')
        validationErrors.push(
          `Invalid data input definition ${relative(process.cwd(), sourcePath)}: ${details}`
        )
      }
      return value
    }
  ...
  const agentFiles = listFilesWithExtensions(join(REGISTRY_ROOT, 'agents'), [
    '.json',
    '.yaml',
    '.yml'
  ])
  const agents: RegistryAgentDefinition[] = agentFiles.map((filePath) => {
    ...
    if (!valid) {
      const details = (validateAgent.errors ?? [])
        ...
        .join('; ')
      validationErrors.push(
        `Invalid agent template ${relative(REGISTRY_ROOT, filePath)}: ${details}`
      )
    }
  ...
  const directorApprovalWarnings = skills
    .map((skill) => {
      const haystack = normalizeReference(
        `${skill.id} ${skill.title} ${skill.tags.join(' ')} ${skill.content}`
      )
      const hasProductTag = skill.tags.some((tag) => normalizeReference(tag) === 'product-specific')
      const hasContextSignal = productContextKeywords.some(
  ...
  const workflowFiles = listFilesWithExtensions(join(REGISTRY_ROOT, 'workflows'), [
    '.json',
    '.yaml',
    '.yml'
  ])
  ...
  const workflows = baseWorkflows.map((workflow) =>
    injectCompanyProtocolsIntoWorkflow(workflow, businessContext)
  )
  ...
  const resolvedAgents: RegistryResolvedAgent[] = agents.map((agent) => {
    const resolvedSkills = agent.skills
      .map((reference) => {
        ...
        if (!resolved) {
          validationErrors.push(
            `Invalid agent template agents/${agent.uid}: missing skill ${reference}`
          )
          return null
        }
        return resolved
    ...
    const resolvedKpis = agent.kpis
      .map((reference) => {
        ...
        if (!resolved) {
          validationErrors.push(
            `Invalid agent template agents/${agent.uid}: missing KPI ${reference}`
          )
          return null
        }
        return resolved
    ...
    const resolvedDataInputs = agent.data_requirements
      .map((reference) => {
        const resolved = resolveByReference(reference, dataInputs, (entry) => [
          entry.uid,
          entry.name
        ])
        if (!resolved) {
          validationErrors.push(
            `Invalid agent template agents/${agent.uid}: missing data input ${reference}`
          )
          return null
        }
        return resolved
  ...
  const intelligence: RegistryAgentIntelligence[] = agents.map((agent) => {
    ...
    const declaredWorkflowRefs = new Set(
      (agent.workflows ?? []).map((workflowRef) => normalizeReference(workflowRef))
    )
    const selectedAgentWorkflows = (agent.workflows ?? [])
      .map((workflowRef) => {
        ...
        const resolved = agentWorkflows.find(
          (workflow) => normalizeReference(workflow.id) === normalizedWorkflowRef
        )
        if (!resolved) {
    ...
    for (const workflow of agentWorkflows) {
      ...
      if (declaredWorkflowRefs.size > 0 && !declaredWorkflowRefs.has(normalizedWorkflowId)) {
        validationErrors.push(
          `Workflow ${workflow.id} exists for agent ${agent.uid} but is not declared in workflows[]`
        )
      }
    }
  ...
  const companyContextAsText =
    typeof company.context === 'string' ? company.context : JSON.stringify(company.context)
  const companyDraftWithContext = upsertDraftField(
    companyCoreDraft,
    'company_vision',
    company.vision
  )
  const companyDraftWithStage = upsertDraftField(
    companyDraftWithContext,
    'company_context',
    companyContextAsText
  )
  const companyDraftWithValues = upsertDraftField(
    companyDraftWithStage,
    'core_values',
    company.core_values.join(', ')
  )
  ...
  const globalAssetsWithSkills = upsertDraftField(
    globalAssetsDraft,
    'approved_skills',
    skills.map((entry) => entry.id).join(', ')
  )
  const globalAssetsWithKpis = upsertDraftField(
    globalAssetsWithSkills,
    'approved_kpis',
    kpis.map((entry) => entry.uid).join(', ')
  )
  const globalAssetsWithProtocols = upsertDraftField(
    globalAssetsWithKpis,
    'approved_protocols',
    protocolDocs.map((entry) => entry.id).join(', ')
  )
  const globalAssetsAligned = upsertDraftField(
    globalAssetsWithProtocols,
    'approved_data_inputs',
    dataInputs.map((entry) => entry.uid).join(', ')
  )
