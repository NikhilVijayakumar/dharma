...
This document defines the standard for the Skill entity within Dharma's agent-based execution architecture. Skill Documentation describes the smallest executable unit of capability — a prompt, a script, and worked examples, bound to exactly one responsibility — that an Agent composes to act.
...
A Skill bundles three things around one declared responsibility: a Prompt (semantic instruction, used when the responsibility requires LLM reasoning), a Script (deterministic executable, used when the responsibility is mechanical), and an Example Set (worked cases showing correct invocation and output). A Skill is invoked by an Agent within a Task step and returns output conforming to its declared Invocation Contract.

### Structural Approach

The Skill Model is the execution primitive beneath the Agent Model. Agents do not act directly; they act only through invoking Skills. A single Skill may support both a Prompt path and a Script path for the same responsibility, letting the runtime pick the deterministic path when available and fall back to the semantic path otherwise.
...
│  Prompt (semantic path)                        │
│  Script (deterministic path)                   │
│  Example Set (worked cases)                    │
...
### Prompt
- **Responsibility:** Holds the semantic instruction template used when this Skill's responsibility requires LLM reasoning.
- **Ownership:** Prompt template text and variable slots.
- **Interfaces:** Consumed by the Prompt/LLM Runtime; receives the Skill's Invocation Contract input.

### Script
- **Responsibility:** Holds the deterministic executable used when this Skill's responsibility is fully mechanical.
- **Ownership:** Script logic.
- **Interfaces:** Consumed by the Script Runtime; receives the Skill's Invocation Contract input.

### Example Set
- **Responsibility:** Demonstrates correct invocation and expected output for calibration.
- **Ownership:** Worked input/output pairs.
- **Interfaces:** Read by authors and reviewers; may be surfaced to the Prompt path as few-shot context.
...
|---|---|
| Skill Definition, Prompt, Script, Example Set | Skill authoring (fixed once approved) |
...
### One Skill, Two Execution Paths
- **Context:** Samgraha splits deterministic (script) and semantic (prompt) execution at the Usecase-step level, tying each to one specific step.
- **Decision:** A single Skill may offer both a Prompt path and a Script path for the same declared responsibility.
- **Alternatives Considered:** Separate Skill types for "deterministic skill" and "semantic skill."
- **Rejection Reason:** Splitting by execution mechanism instead of by responsibility fragments what is conceptually one capability and prevents reuse when a deterministic implementation later becomes available for a previously prompt-only Skill.
- **Architectural Goal:** Reusable, mechanism-agnostic capability.
