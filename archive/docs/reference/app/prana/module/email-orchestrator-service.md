# EmailOrchestrator Service ΓÇö Atomic Feature Specification
...
- **KPIs:** Not directly measured ΓÇö delegates to `draft-cycle-time` (measured at module level).
...
2. Resolve target agent(s) based on department tags using Agent Directory Registry:
   - Finance ΓåÆ `nora`, Compliance ΓåÆ `eva`, HR ΓåÆ `lina`, Operations ΓåÆ `elina`, Strategy ΓåÆ `arya`, Design ΓåÆ `sofia`, General ΓåÆ `mira`.
...
|:---------|:----------|:------------|
| `email:configure-account` | Renderer ΓåÆ Main | Add/edit/remove a Director email account |
| `email:list-accounts` | Renderer ΓåÆ Main | List all configured email accounts |
| `email:fetch-unread` | Renderer ΓåÆ Main | Manual trigger for a specific account |
| `email:fetch-all-accounts` | Renderer ΓåÆ Main | Trigger intake across all active accounts |
| `email:get-triage-summary` | Renderer ΓåÆ Main | Retrieve Mira's classified summary for Director |
| `email:select-for-draft` | Renderer ΓåÆ Main | Director marks specific emails for response drafting |
| `email:compose-new-draft` | Renderer ΓåÆ Main | Director instructs a fresh outbound draft (not a reply) |
| `email:contribute-to-draft` | Agent ΓåÆ Service | Agent submits their section to a shared draft |
| `email:get-draft` | Renderer ΓåÆ Main | Retrieve shared draft with contributor attribution |
| `email:approve-draft` | Renderer ΓåÆ Main | Director approves ΓåÆ saves to Vault (never sends) |
| `email:mark-batch-read` | Renderer ΓåÆ Main | "Clear Desk" with Director confirmation gate |
| `email:get-batch-history` | Renderer ΓåÆ Main | List past intake batches with status |
...
interface EmailActionItem {
  ...
  emailUid: number;         // IMAP UID ΓÇö UNIQUE per account
  subject: string;
  sender: string;
  senderDomain: string;
  receivedAt: string;
...
- **OpenCLAW:** Enforces pre-flight security on all draft content before save ΓÇö scans for internal Vault key patterns, unapproved data references, and policy violations.
...
- **SQLite:** `email_accounts`, `email_action_items`, `email_drafts`, `email_draft_contributions`, `email_read_cursors` ΓÇö all operational state.
