# Email Management ΓÇö Atomic Feature Specification

## 1. Single Reason to Change (SRP)
This document handles updates **exclusively** related to the intake, triage, collaborative drafting, and local persistence of email correspondence for the Director's desk. It never handles sending ΓÇö all drafts are saved locally for the Director to send manually from their email client.

## 2. Input Data Required
- **Mailbox Accounts:** Configurable list of Director email accounts (Gmail, Outlook, generic IMAP).
- **Credentials:** IMAP host, port, auth credentials ΓÇö runtime-local only (never persisted to Vault).
...
- Accounts are polled independently ΓÇö each has its own cron job registered as `EMAIL_HEARTBEAT_<accountId>`.
- Account credentials remain runtime-local and excluded from Vault payload.

## 4. Email Lifecycle State Machine

```
INTAKE ΓåÆ TRIAGE_SUMMARY ΓåÆ DIRECTOR_SELECTS ΓåÆ MULTI_ROLE_INPUT ΓåÆ DIRECTOR_REVIEW ΓåÆ DRAFT_SAVED
```
...
### New Email Drafting
The Director can also instruct Mira to compose a **fresh outbound draft** (not a reply) with specific details. The system routes to relevant agents for data, assembles the draft, and saves it for Director review. These follow: `MULTI_ROLE_INPUT ΓåÆ DIRECTOR_REVIEW ΓåÆ DRAFT_SAVED`.
...
- **OpenCLAW:** Enforces pre-flight security on draft content ΓÇö no internal Vault keys, no unapproved attachments, no confidential data leakage in outgoing drafts.
...
- Auto-classified as `ARCHIVE` ΓÇö stored in intake log but never enters Director's triage summary.
...
- **Schedule:** Configurable ΓÇö `once_daily` (default 07:00) or `twice_daily` (07:00 + 19:00), times adjustable.
- **Failover / Catch-up:** Uses existing `queue-orchestration.md` guarantees ΓÇö INTERRUPTED jobs re-enqueued on restart, MISSED schedules fire immediately on boot.
- **Deduplication:** `lastReadUID` cursor + `emailUid` UNIQUE constraint on ActionItems prevents duplicate processing.

## 11. **HARD CONSTRAINT ΓÇö READ & DRAFT ONLY**
