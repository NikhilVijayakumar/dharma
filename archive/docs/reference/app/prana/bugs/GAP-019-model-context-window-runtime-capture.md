...
- If user selects a local model with custom 16K context limit configured in LM Studio, system still uses 32K default ΓåÆ context threshold may be reached prematurely or allow dangerous overflow.
...
- `Model Context Window (tokens)` ΓÇö text input, optional.
- `Reserved Output Tokens` ΓÇö text input, optional.
...
6. Update Docs.
7. Test end-to-end: onboarding ΓåÆ settings ΓåÆ context engine budget calculation.
...
- **docs/bugs/GAP-019-model-context-window-runtime-capture.md**: Comprehensive gap document and implementation tracking.
